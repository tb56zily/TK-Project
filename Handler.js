 ...
 function memberinfo(req,res){
   ...
   console.log("we are in memberinfo method");
    ...
    function getSubmissionById(req,res){
      console.log("Retrieving submission with id-"+req.query._id);
       Submission.findSubmissionById(req.query._id, function(err, submission) {
          console.log("submission data:" + submission);
          if (err) throw err;
          if (!submission) {
            console.log("Error Retrieving submission with id-"+req.query._id);
            return res.status(403).send({success: false, msg: 'No records found.'});
          } else {
            console.log("Successfully Retrieved submission with id-"+req.query._id);
            res.json({success: true, msg: 'Records found for id ' + submission + '!'});
          }
        });
    }
    ...
    function assignReviewer(req,res){
      console.log("assigning a new Reviewer...");
      if (!req.body.assignment_id) {
        console.log("No assigned reviewer");
        res.json({success: false, msg: 'Please assign reviewer.'});
      }
      else {
